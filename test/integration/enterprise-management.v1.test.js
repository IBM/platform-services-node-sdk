...
const am_coe_v2_account_apis_helper = require('../integration/am_coe_v2_account_apis');
...
const activation_db_name = config.ACTIVATION_DB_NAME;
const am_host = config.AM_HOST;
const iam_host = config.IAM_HOST;
const iam_basic_auth = config.IAM_BASIC_AUTH;
const iam_api_key = config.IAM_API_KEY;
...
describe('EnterpriseManagementV1_integration', () => {
  ...
  it('Create a Standard Account - should generate IAM service token', done => {
    am_coe_v2_account_apis_helper.generate_iam_service_token(iam_host, iam_basic_auth, iam_api_key, (e, token) => {
  ...
  it('Create a Standard Account - calls POST /coe/v2/accounts with BSS token', done => {
    ...
    am_coe_v2_account_apis_helper.post_am_coe_v2_accounts(am_host, payload, am_service_iam_token, (e, r, b) => {
  ...
  it('Create a Standard Account - waits until activation token is generated', done => {
    const db_activation_token = am_coe_v2_account_apis_helper.fetch_db_activation_token(db_url, activation_db_name, db_user, db_pass, account_info.email);
  ...
  it('Create a Standard Account - calls GET /coe/v2/accounts/:account_id with BSS token', done => {
    am_coe_v2_account_apis_helper.get_am_coe_v2_account_by_id(am_host, account_info.account_id, am_service_iam_token, (e, r, b) => {
      done();
    });
  });

  it('Create a Standard Account - calls GET /coe/v2/accounts/verify?email=******&token=****** to activate the account', done => {
    am_coe_v2_account_apis_helper.get_am_coe_v2_accounts_verify(am_host, account_info.email, activation_token, (e, r, b) => {
      done();
    });
  });
  ...
  it('Create a Subscription Account - convert account to subscription', done => {
    ...
    am_coe_v2_account_apis_helper.patch_am_coe_v2_account_subscription(am_host, account_info.account_id, subscription_id, payload_to_convert, am_service_iam_token, null, (e, r, b) => {
      done();
    });
  });
